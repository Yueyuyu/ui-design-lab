...
export async function starterFiles(target, selectedId, {
  ...
  const kit = kitId ? kits.find(item => item.id === kitId && item.preferred === selectedId) : null;
  if(kitId && !kit) throw new Error('场景与套系不匹配：' + kitId);
  const chosen = selectedId ? manifests.filter(m => m.id === selectedId) : manifests;
  const needsWorkflow = chosen.some(m => !m.starter?.component);
  ...
  await mkdir(resolve(target, needsWorkflow ? "src/workbench" : "src"), {
    recursive: true
  });
  if (needsWorkflow) {
  ...
  const configs = chosen.map((m, i) => {
    if (m.starter?.component) return JSON.stringify({ id:m.id, name:m.displayName, props:m.starter.props ?? {} }).replace(/}$/, ',Workspace:Kit' + i + '.' + m.starter.component + '}');
  ...
  let main = 'import {useState} from "react";\nimport {createRoot} from "react-dom/client";\nimport {WorkflowDemo} from "./workbench/WorkflowDemo.jsx";\nimport "./style.css";\n' + imports + '\nconst systems=[' + configs + '];\nfunction App(){const [id,setId]=useState(systems[0].id),[kind,setKind]=useState("tasks");const suite=systems.find(s=>s.id===id);return <main><h1>独立项目工作台</h1><p>安装的 ESM 组件包 · 无 Gallery 依赖 · 明确的本地模拟接口</p><label>设计套系<select value={id} onChange={e=>setId(e.target.value)}>{systems.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>{!suite.Workspace && <label>业务场景<select value={kind} onChange={e=>setKind(e.target.value)}><option value="tasks">任务与用量</option><option value="research">研究与内容</option><option value="reports">运营报表</option></select></label>}<section data-ui-system={id} data-density="comfortable">{suite.Workspace ? <suite.Workspace key={id} {...suite.props}/> : <WorkflowDemo key={id+kind} ui={suite.ui} suiteId={id} kind={kind}/>}</section></main>;}\ncreateRoot(document.getElementById("root")).render(<App/>);\n';
  if (!needsWorkflow) {
    main = main.replace('import {WorkflowDemo} from "./workbench/WorkflowDemo.jsx";\n', '')
      .replace(',[kind,setKind]=useState("tasks")', '')
      .replace(/\{!suite.Workspace && <label>业务场景[\s\S]*?<\/label>\}/, '')
      .replace('<WorkflowDemo key={id+kind} ui={suite.ui} suiteId={id} kind={kind}/>', 'null');
  }
  if (kit && ['tasks','research','reports'].includes(kit.id)) main = main.replace('useState("tasks")', 'useState("' + kit.id + '")');
  await writeFile(resolve(target, "src/main.jsx"), main);
  const integration = [];
  for (const suite of chosen) {
    try { integration.push(await readFile(resolve(root, 'systems', suite.id, 'INTEGRATION.md'), 'utf8')); }
    catch(error) { if(error.code !== 'ENOENT') throw error; }
  }
  await writeFile(resolve(target, 'integration.md'), (kit ? '# 所选场景：' + kit.title + '\n\n' : '') + (integration.join('\n\n') || '在 src/main.jsx 配置本套组件。读取安装包 systems/<suite-id>/API.md 和 web/index.d.ts，使用自己的数据与保存服务。') + '\n');
  ...
  await writeFile(resolve(target, "README.md"), `# 独立工作台\n\nUI Design Lab ${packageManifest.version} · Node.js 22+ · React 19.2\n\n组件包已随 vendor/ 交付，可将整个目录复制到其他机器。\n\n1. 在本目录执行 npm install。\n2. 执行 npm run dev，打开终端显示的地址。\n${needsWorkflow ? "3. 修改 src/workbench/demo-data.js 中的任务名，再查看列表与详情。\n4. 修改 src/workbench/WorkflowDemo.jsx 的列定义，增加业务字段。" : "3. 在页面中试用所选套系的工作台，测试编辑与保存。\n4. 在 src/main.jsx 配置页面参数，按 integration.md 接入数据与保存服务；Folio 可用 storageKey 隔离本地工作区。"}\n5. 执行 npm run build，部署 dist/。\n\n${needsWorkflow ? "通用业务接口边界见 WorkflowDemo 的本地模拟实现。" : "专属工作台未连接生产服务。Clearline / Signal 默认仅修改内存；Folio 使用本地存储，详见各套 API。"}没有生产认证或支付。组件来自已安装的 ui-design-lab 子路径，数据与页面文件由你的项目持有。保留 LICENSE 和 NOTICE.md。\n`);
