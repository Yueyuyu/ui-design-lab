...
import {
  QuietButton as Button, QuietCard as Panel, QuietField as Field, QuietToggle as Toggle,
  QuietTable as Table, QuietBarChart as Chart, QuietStatusChip as Badge,
  QuietNotification as Notification, QuietEmptyState as EmptyState,
} from "../web/index.js";
import "../foundations/tokens.css";
import "../web/components.css";

export function Status({ suite }) {
  return <Badge tone="success" dot>{({ stable: "稳定", experimental: "实验", draft: "草稿", deprecated: "已弃用" })[suite.status]}</Badge>;
}
...
function Metrics({ data, visualState }) {
  return <div className="comparison-metrics">{data.metrics.map((metric) => {
    ...
    return <div className="comparison-qw-metric" key={metric.id} data-state={visualState}>
...
function DataTable({ data, visualState, onRetry, onNotify }) {
  const renderStatus = (value) => <Badge tone={value === "已确认" ? "success" : value === "待复核" ? "running" : "attention"}>{value}</Badge>;
  const columns = [
...
export function Scene({ scenarioId, ...props }) {
  if (scenarioId === "monthly-review") return <>
    <Panel eyebrow="MONTHLY REVIEW" title="月度经营复盘" action={<Badge tone="running">待确认</Badge>}>
