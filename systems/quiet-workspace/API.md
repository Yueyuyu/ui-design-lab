# Quiet Workspace 组件 API

入口：`ui-design-lab/quiet-workspace`。必须导入同子路径下的 tokens.css 和 components.css，并使用 `data-ui-system="quiet-workspace"` 包装页面。完整参数类型见 [web/index.d.ts](web/index.d.ts)，运行示例见 [examples/consumer](../../examples/consumer)。

## 基础组件

| 组件 | 主要参数 | 事件与说明 |
|---|---|---|
| QuietButton | children、variant、disabled、loading、error、icon | onClick 为 React 鼠标事件；loading 禁止重复提交 |
| QuietIconButton | label、icon、disabled、loading | label 是必须提供的可访问名称 |
| QuietField | label、value/defaultValue、hint、error | onChange(event)，通过 event.target.value 读取值 |
| QuietSelect | label、options、value | options 为 {value,label}[]，onChange(event) |
| QuietToggle | label、description、checked | onChange(boolean)，由业务方更新 checked |
| QuietCard | title、eyebrow、action、children | 使用容器承载自己的内容状态 |
| QuietStatusChip | children、tone、dot | 使用 state 显示静态状态。tone 为 neutral/running/attention/success/offline |
| QuietDialog | open、onOpenChange、title、description、children、actions | 受控弹窗；Escape/背景/关闭按钮请求 onOpenChange(false) |
| QuietNotification | title、description、tone、actionLabel | onAction、onDismiss；tone 为 info/success/warning/error |
| QuietEmptyState | title、description、icon、actionLabel | onAction 提供下一步；error 表达失败与恢复 |
| QuietTable | caption、columns、rows、state | onRowActivate(row)、onRetry；columns 的 render 接收整行 row |
| QuietBarChart | title、data、unit、state | data 为 {label,value}[]；onRetry 用于错误恢复 |
| QuietTaskLight | tasks、defaultOpen、onRetry | tasks 项包含 id/title/detail/state/time，固定紧凑密度 |
| QuietQuotaPill | remainingPercent、resetText、panelAlign | onRefresh、onExit；null 表示未知额度，固定紧凑密度 |
| QuietWorkspacePreview | 无参数 | 页面模式演示，不连接真实业务 |
...
<QuietButton onClick={() => setOpen(true)}>编辑</QuietButton>
<QuietDialog
  open={open}
  onOpenChange={setOpen}
  title="编辑项目"
  actions={<QuietButton onClick={() => setOpen(false)}>保存</QuietButton>}
>
  <QuietField label="项目名称" value={name} onChange={(event) => setName(event.target.value)} />
</QuietDialog>
```
