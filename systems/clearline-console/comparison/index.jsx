import {RecordWorkflow, RecordDetail} from "./RecordWorkflow.jsx";
...
export function Scene({scenarioId,...p}) {
 if(scenarioId==="record-workflow")return <RecordWorkflow {...p}/>;
...
export function Module({moduleId,...p}){if(moduleId==="record-detail")return <RecordDetail {...p}/>;return moduleId==="metrics"?<Metrics {...p}/>:moduleId==="chart"?<Chart {...p}/>:moduleId==="form"?<Form {...p}/>:<div className="workbench-actions"><U.Button onClick={p.onCopy}>确认复盘</U.Button><U.Button variant="secondary" onClick={p.onCopy}>复制指令</U.Button><U.Button loading>正在处理</U.Button><U.Button disabled>暂不可用</U.Button></div>;}
