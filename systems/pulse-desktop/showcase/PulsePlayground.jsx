...
export function PulsePlayground() {
  ...
  const [notice, setNotice] = useState('整个胶囊都可拖动，移动超过 6px 才开始；自动贴边默认关闭，可在详情中开启。');
  ...
  const geometry = () => {
    const outer=widget.current.getBoundingClientRect(),rail=widget.current.querySelector('.pd-rail,.pd-edge').getBoundingClientRect();
    return {bounds:{width:stage.current.clientWidth,height:stage.current.clientHeight},size:{width:rail.width,height:rail.height},offset:{x:rail.x-outer.x,y:rail.y-outer.y}};
  };
  const reachable = (position,{bounds,size,offset}) => ({
    x:Math.max(Math.min(16*scale,size.width)-size.width-offset.x,Math.min(position.x,bounds.width-Math.min(16*scale,size.width)-offset.x)),
    y:Math.max(Math.min(24*scale,size.height)-size.height-offset.y,Math.min(position.y,bounds.height-Math.min(24*scale,size.height)-offset.y)),
  });
  ...
  useLayoutEffect(() => {
    ...
    const fit = () => {
      const layout=geometry(),{bounds,size,offset}=layout;
      place(reachable({ ...currentPosition.current, ...(mode === 'docked' ? { x: side === 'right' ? bounds.width-size.width-offset.x : -offset.x } : {}) },layout));
    };
    ...
    if(drag.current)drag.current.start={...drag.current.start,x:drag.current.start.x+offset(before.mode,before.side,before.scale)-offset(mode,side,scale)};
  ...
  const startDrag = event => {
    ...
    drag.current = { pointer: event.pointerId, x: event.clientX, y: event.clientY, start: currentPosition.current };
  };
  const moveDrag = event => {
    const gesture = drag.current;
    if (!gesture || event.pointerId !== gesture.pointer) return;
    const dx = event.clientX - gesture.x, dy = event.clientY - gesture.y;
    place(reachable({ x: gesture.start.x + dx, y: gesture.start.y + dy }, geometry()));
  };
  const endDrag = event => {
    const gesture = drag.current;
    if (!gesture || event.pointerId !== gesture.pointer) return;
    ...
    const { bounds, size, offset } = geometry();
    const nextSide = autoDock?dockingSide({x:currentPosition.current.x+offset.x,y:currentPosition.current.y+offset.y}, bounds, size):null;
  ...
  return <section className="pd-playground">
    ...
    <div className="pd-stage-scroll"><div ref={stage} className="pd-stage" data-wallpaper={wallpaper} style={{ minWidth: Math.max(0, 384 * scale), minHeight: Math.max(640, 630 * scale) }} aria-label="桌面模拟画布">
      ...
      <div ref={widget} className="pd-positioner" style={{ left: position.x, top: position.y }} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} onKeyDown={event => {
        ...
        ]} pinned={pinned} autoDock={autoDock} onAutoDockChange={setAutoDock} motionEnabled={motionEnabled} dockSide={side} onModeChange={setMode} onPinnedChange={setPinned} onDragStart={startDrag}
    ...
    <div className="pd-notes"><p><strong>01 / 不占你的工作区</strong><br/>整个胶囊都可拖动，轻点查看详情；自动贴边可开关。只保留能重新抓住的边缘。</p><p><strong>02 / 关注不会消失</strong><br/>星标选择重点任务；固定面板后点击画布也不收起，Escape 仍可关闭。</p><p><strong>03 / 读数不冒充进度</strong><br/>圆环只表示额度。读取失败显示未知，不显示零或绿色健康状态。</p></div>
  </section>;
}
