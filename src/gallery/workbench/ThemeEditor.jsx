...
export function ThemeEditor({
 ...
 <ThemeControls ui={U} suite={suite} theme={theme} onChange={commit}/>
 <div className="workbench-actions"><U.Button variant="secondary" disabled={!position} onClick={() => setPosition(position - 1)}>撤销</U.Button><U.Button variant="secondary" disabled={position === history.length - 1} onClick={() => setPosition(position + 1)}>重做</U.Button><U.Button variant="secondary" onClick={() => commit(defaults(suite))}>恢复默认</U.Button><U.Button variant="secondary" onClick={() => {
            try {
          ...
          }}>保存主题</U.Button><U.Button variant="secondary" onClick={() => {
            try {
 ...
 <div className="workbench-actions"><U.Button variant="secondary" onClick={() => {
            downloadText(suiteId + "-theme.json", JSON.stringify(theme, null, 2), "application/json");
            recordEvent("theme_export", {
              suiteId
            });
          }}>导出 JSON</U.Button><U.Button variant="secondary" onClick={() => {
            downloadText(suiteId + "-theme.css", themeCSS(theme), "text/css");
            recordEvent("theme_export", {
              suiteId
            });
          }}>导出 CSS</U.Button><U.Button variant="secondary" onClick={() => setJSON(JSON.stringify(theme, null, 2))}>显示配置</U.Button><U.Button variant="secondary" onClick={async () => {
            try {
          ...
          }}>复制主题分享链接</U.Button></div>
 <U.Textarea label="主题 JSON" aria-label="主题 JSON" value={json} onChange={e => setJSON(e.target.value)} rows={7} /><U.Button variant="secondary" onClick={() => read(json)}>导入 JSON</U.Button><pre>{themeCSS(theme)}</pre><p>应用 CSS 时给套系根元素添加 data-theme="custom"，密度使用 data-density="{theme.density}"。</p>
