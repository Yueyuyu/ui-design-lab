...
import { validateRecordWorkflow } from "./record-workflow.js";
const scenarios=["record-workflow","monthly-review","data-table","settings-form","detail-page","empty-state"];
export function validateComparison(value) {
 ...
 if(value.scenarioId === "record-workflow" || value.workflow) validateRecordWorkflow(value.workflow);
 return value;
}
export function ComparisonPersistence({snapshot,onRestore,onNotify}) {
 ...
 return <details className="comparison-persistence comparison-tools"><summary>保存、恢复与分享比较配置</summary><div className="comparison-persistence__body"><p>本机保存和 JSON 包含当前演示表单内容。分享链接会包含这些内容，请先清除不适合分享的信息。</p><div className="workbench-actions"><button type="button" onClick={()=>{try{localStorage.setItem("ui-lab-comparison",JSON.stringify(snapshot));setMessage("比较已保存到本机");}catch{setMessage("本地存储不可用，请导出配置");}}}>保存比较</button><button type="button" onClick={()=>{try{restore(localStorage.getItem("ui-lab-comparison"));}catch{setMessage("本地存储不可用");}}}>恢复比较</button><button type="button" onClick={()=>downloadText("comparison.json",JSON.stringify(snapshot,null,2),"application/json")}>导出比较 JSON</button><button type="button" onClick={async()=>{try{const url=new URL(location.href);url.hash="compare?comparison="+encodeURIComponent(JSON.stringify(snapshot));if(url.href.length>12000)throw Error();await copyText(url.href);onNotify?.("包含当前演示内容的分享链接已复制");}catch{setMessage("复制失败或内容过长，请导出 JSON");}}}>复制含当前内容的分享链接</button><button type="button" onClick={()=>setInput(JSON.stringify(snapshot,null,2))}>显示当前配置</button></div><label>比较 JSON<textarea rows={5} value={input} onChange={e=>setInput(e.target.value)}/></label><button type="button" onClick={()=>restore(input)}>导入比较</button><p role="status">{message}</p></div></details>;
}
